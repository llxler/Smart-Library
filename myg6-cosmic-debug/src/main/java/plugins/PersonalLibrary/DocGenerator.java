...
public class DocGenerator implements IGPTAction {
    @Override
        ...
        if ("GENERATE_FILE_DOC".equalsIgnoreCase(action)) {
            ...
            try {
                ...
                for (int i = 0 ;i < jsonArrayData.size() && i < 6; i++) {
                    ...
                    // 填写书名
                    String l1 = "Book" + i;
                    fileContent = fileContent.replaceAll(l1, jsonArraySingle.getString(0));

                    // 填写类别
                    String l2 = "Type" + i;
                    fileContent = fileContent.replaceAll(l2, jsonArraySingle.getString(1));

                    // 填写借阅时间
                    String l3 = "Borrow" + i;
                    fileContent = fileContent.replaceAll(l3, jsonArraySingle.getString(2).substring(0, 10));

                    // 填写归还时间
                    String l4 = "Return" + i;
                    fileContent = fileContent.replaceAll(l4, jsonArraySingle.getString(3).substring(0, 10));
                }
                // 替代总结
                fileContent = fileContent.replaceAll("dayEvaluate", params.get("dayPrompt"));
                System.out.println("fileContent: " + fileContent);
