 ...
 * 动态表单插件
 */
public class buttom_Gpt_MindMap extends AbstractFormPlugin implements Plugin {
    @Override
    ...
    public void itemClick(ItemClickEvent e) {
        super.itemClick(e);
        if (e.getItemKey().equalsIgnoreCase("myg6_buttonap_gpt")) {
            // 获取日任务信息，并且以JSON字符串的形式展现
            JSONObject jsonResultObject = new JSONObject();
            // todo: 从数据模型中获取数据
            jsonResultObject.put("bookName", this.getModel().getValue("myg6_bookinfo").toString());
    ...
    public void afterBindData(EventObject eventObject) {
        Markdown mk = this.getView().getControl("myg6_txt");
        mk.setText(this.getModel().getValue("myg6_md_txt").toString());
    }
}