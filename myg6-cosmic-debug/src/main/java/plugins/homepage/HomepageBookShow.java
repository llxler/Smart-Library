...
public class HomepageBookShow extends AbstractFormPlugin implements ListboxClickListener {
                ...
                for (int i = 0;i < 5;++i) {
                    ...
                    if (StringUtils.equals(label, source.getKey())) {
                        ...
                        if (j == 0) {
                            ...
                            // 直接暴力讨论
                            if (i == 0) {
                                bookname = bookTitles.get(bookTitles.size() - 2);
                            } else if (i == 1) {
    ...
    private void setSmall() {
        int i = 0;
        // 先set一个末尾倒数第二个
        String title1 = bookTitles.get(bookTitles.size() - 2);
        Map<String, String> info1 = booksInfo.get(title1);
        String coverUrl1 = info1.keySet().iterator().next();
        String borrowCountStr1 = info1.values().iterator().next();
        Label label11 = this.getView().getControl("myg6_labelaps1");
        Label label22 = this.getView().getControl("myg6_labelaps11");
        label11.setText(title1);
        label22.setText("借阅次数:" + borrowCountStr1);
        this.getModel().setValue("myg6_picturefield06", coverUrl1);
